...
import { Monitor, CircleDot, Clock, ShieldCheck, AlertCircle, CheckCircle2, Crown, Star } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      {profile && (
        <motion.div
          ...
          className={`relative overflow-hidden bg-gradient-to-r ${getMembershipGradient(profile.membershipTier)} rounded-2xl p-4 text-white shadow-lg`}
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs opacity-90">Welcome back, {profile.name}!</p>
                <p className="font-bold text-lg">{profile.membershipTier?.toUpperCase() || "FREE"} Member</p>
              </div>
            </div>
          ...
          <div className="mt-3 flex gap-2 flex-wrap">
            {getMembershipBenefits(profile.membershipTier).map((benefit, i) => (
              <Badge key={i} variant="secondary" className="bg-white/20 text-white border-white/30 text-[10px]">
                {benefit}
              </Badge>
            ))}
          </div>
      ...
      <section className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
        <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
          <ShieldCheck size={18} className="text-blue-600 dark:text-blue-400" /> Why Reserve Ahead?
        </h3>
        <ul className="grid grid-cols-1 gap-3">
          {[
            ...
            <li key={i} className="flex gap-3 items-start">
              <div className="mt-0.5 p-1 bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-400 rounded-md">
                <benefit.icon size={14} />
              </div>
              <div>
                <p className="text-xs font-bold text-slate-700 dark:text-slate-300">{benefit.title}</p>
...
function UnitCard({ unit, index }: { unit: Unit; index: number }) {
  ...
  return (
    <motion.div
      ...
      className={`relative overflow-hidden bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow`}
      role="listitem"
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div 
            className={`p-3 rounded-xl ${unit.status === 'available' ? 'bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500'}`}
            aria-hidden="true"
          >
            <Monitor size={24} />
          </div>
          <div>
            <h3 id={`unit-title-${unit.id}`} className="font-bold text-lg dark:text-slate-100">{unit.name}</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400" aria-label={`Console type ${unit.type} at ${unit.pricePerHour} dollars per hour`}>
      ...
      {unit.status === 'rented' && (
        <div className="mt-4 pt-3 border-t border-dashed border-slate-100 dark:border-slate-800 flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 italic" aria-live="polite">
